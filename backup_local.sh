...
set -e
# Считать команду с пайпом (|) неуспешной, если любая из ее частей завершилась с ошибкой
set -o pipefail
...
if [ -f "$ENV_FILE" ]; then
  ...
  # Используем 'set -a' для автоматического экспорта переменных, определенных в .env
  set -a
  source "$ENV_FILE"
  # Отключаем автоматический экспорт
  set +a
else
...
# --- НАСТРОЙКИ (Значения по умолчанию или переопределения) ---

# Имя или ID вашего PostgreSQL контейнера (установлено из вашего сообщения)
# Можно переопределить через переменную окружения PG_CONTAINER_NAME перед запуском
PG_CONTAINER_NAME="${PG_CONTAINER_NAME:-project-practice-java-backend-postgres-1}"

# Имя пользователя PostgreSQL (ожидается из .env)
PG_USER="${POSTGRES_USER}"

# Имя базы данных для бэкапа (ожидается из .env)
PG_DB="${POSTGRES_DB}"

# Пароль пользователя PostgreSQL (ожидается из .env)
PG_PASSWORD="${POSTGRES_PASSWORD}" # Эта переменная используется ниже

# Имя ИМЕНОВАННОГО Docker тома (volume), используемого MinIO
# Можно переопределить через переменную окружения MINIO_VOLUME_NAME перед запуском
MINIO_VOLUME_NAME="${MINIO_VOLUME_NAME:-project-practice-java-backend_minio_data}" # Имя из вашего docker-compose для MinIO

# Директория для сохранения бэкапов (относительно места запуска скрипта)
# Можно переопределить через переменную окружения BACKUP_DIR перед запуском
...
# --- Определение пути к данным MinIO ---
...
# Используем PG_PASSWORD, загруженный из .env
docker exec -t \
...
# Опционально: Очистка старых бэкапов
